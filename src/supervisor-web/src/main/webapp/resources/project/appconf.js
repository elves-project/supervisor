...
//新增窗口打开
function adddialog(){
	$(".modal-title").empty().append("<strong>新增应用</strong>");
...
function editdialog(id){
	$("#did").val(id);
	$("#uploadmodal .modal-title").empty().append("<strong>版本上传</strong>");
	//$("#forminfo").attr("action",""); //补充新增的action指向：需改动
	$.ajax({
	    url:'',						  //补充新增的url地址：需改动
	    type:'POST', 				  //GET
	...
	$('#uploadmodal').modal('toggle');	//补充完毕后把该行注释，把ajax中的注释打开
}

function infodialogdisplay(id){
	//获取id并补充弹窗表格中的信息
...
function updatedialogdisplay(id){
	//获取id并补充弹窗表格中的信息
	$('.updateinfodialog').modal('toggle');
}

function updateversion(id,vn){
	var r=confirm("确认修改该版本号？");
	if (r==true){
		//操作
		alert("修改成功");
	}
